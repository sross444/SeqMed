...
class SeqMed(nn.Module):
    def __init__(self,n_proc, n_diag, n_meds, dim = 128):
        ...
        self.med_embed = nn.Embedding(num_embeddings = self.nm+2, embedding_dim = dim)
    ...
    def single_step(self, hsr, state = None, med_vec = None):
        ...
        if med_vec is None:
            med_vec = self.med_embed(self.start_token).repeat(
                hsr.shape[0],1).unsqueeze(1) # batch, 1, dim
        else:
            med_vec = self.med_embed(med_vec)
        if state is None:
    ...
    def predict(self, p, d, T=36):
        ...
        for step in range(T):
            ...
            # global pooling...
            med_vec = pred.argmax(dim=1).to('cuda').type(torch.long).unsqueeze(1)
            rv.append(pred)
...
def train_SeqMed(train_DL, val_DL, model, discriminator, epochs = 1, lr = 1e-2, T = 24):
    ...
    for epoch in range(epochs):
        ...
        for p_, d_, m_, mh, n_p, n_d, n_m, n_v in train_DL:
            ...
            in_meds = torch.zeros((b_size, max_meds)).to('cuda').type(torch.long)
            in_meds[:,0] = model.start_token
            
            loss_masks = torch.ones((b_size, max_meds)).to('cuda')
            for b in range(b_size):
                loss_masks[b,(n_meds[b]+1):] = 0
                # random shuffle real meds
                in_meds[b][1:(n_meds[b]+1)] = meds[b][torch.randperm(n_meds[b])]
            ...
            preds = torch.zeros((b_size, max_meds, model.nm+1)).to('cuda')
            hsrs = model.hsr(p_, d_)
            states = torch.zeros((b_size, max_meds+1, 1, model.dim)).to('cuda')
            for s in range(max_meds):
                pred, states_ = model.single_step(hsrs, states[:,s], in_meds[:,s].unsqueeze(1))
                preds[:,s] = pred
                states[:,s+1]=states_
            loss = criterion(torch.sigmoid(preds),targs)* loss_masks.unsqueeze(-1)
            ...
            pairs = torch.stack([
                ...
                targs.reshape((-1,preds.shape[-1]))],dim=1)
            ...
            for s in range(max_meds):
                ...
                preds[:,s] = pred
                states[:,s+1]=states_
            reward = discriminator.reward(preds.reshape((-1,preds.shape[-1])))
        ...
        if js > best_ja:
            torch.save(model.state_dict(), open('SeqMed_teacherforce.model','wb'))
            best_ja = js
